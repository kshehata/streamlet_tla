...
\* These are all helpers to extract all possible QCs
ExtractMessageVNWithType(Messages, type) ==
    { m.viewNum : m \in { m \in Messages : m.type = type } }

FilterMessagesForTypeAndVN(Messages, type, vn) == 
    {m \in Messages : m.type = type /\ m.viewNum = vn}

ExtractSetsByVNForType(Messages, type) == { 
    FilterMessagesForTypeAndVN(Messages, type, vn)
    : vn \in ExtractMessageVNWithType(Messages, type) }

GenerateAllQCForType(Messages, type) == {
    GenerateQC(votes) : votes \in {
        votes \in ExtractSetsByVNForType(Messages, type)
        : CheckVotesForQC(votes)
    }
